...
```
docker build -t registry.lingo.local/service/kkfileview:v4.4.0-alpine-7.2.2 .
```
...
docker run --rm --name kkfileview \
    -p 18012:8012 \
    registry.lingo.local/service/kkfileview:v4.4.0-alpine-7.2.2
```
...
```
docker push registry.lingo.local/service/kkfileview:v4.4.0-alpine-7.2.2
```

**保存镜像**

```
docker save registry.lingo.local/service/kkfileview:v4.4.0-alpine-7.2.2 |
    gzip -c > image-kkfileview_v4.4.0-alpine-7.2.2.tar.gz
```
