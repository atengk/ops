...
export CHAR_FIX_STORAGE=${CHAR_FIX_STORAGE:-1}
...
# 初始化参数
export DMINIT_OPTS=${DMINIT_OPTS:-path=${DATA_PATH} PAGE_SIZE=${PAGE_SIZE} EXTENT_SIZE=${EXTENT_SIZE} CASE_SENSITIVE=${CASE_SENSITIVE} CHAR_FIX_STORAGE=${CHAR_FIX_STORAGE} UNICODE_FLAG=${UNICODE_FLAG} DB_NAME=${DB_NAME} INSTANCE_NAME=${INSTANCE_NAME} PORT_NUM=${PORT_NUM} LOG_SIZE=${LOG_SIZE} BUFFER=${BUFFER} SYSDBA_PWD=${SYSDBA_PWD} SYSAUDITOR_PWD=${SYSAUDITOR_PWD} }
