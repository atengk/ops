...
## 数据库信息
MONGO_HOST=172.16.0.148
MONGO_PORT=27017
MONGO_USER=admin
MONGO_PASS=Admin@123
MONGO_DATABASE=kongyu
## 数据库备份位置
BACKUP_DIR="/data/backups/mongodb"
BACKUP_FILE="${MONGO_DATABASE}_$(date +%Y%m%d%H%M%S)"
## 备份输出日志
...
function create_bakckup() {
	...
	## 开始备份
	/usr/local/bin/mongodump -h ${MONGO_HOST}:${MONGO_PORT} -u ${MONGO_USER} -p ${MONGO_PASS} -d ${MONGO_DATABASE} -o ${BACKUP_FILE} --authenticationDatabase admin &>> ${LOG_FILE}
	## 判断备份是否执行成功
	if [[ "$?" == "0" ]]
	then
...
## 恢复数据库
:<<!
mongorestore -h 172.16.0.148:27017 -u admin -p Admin@123 -d treeFacility /data/backups/mongodb/treeFacility_20220303171931/treeFacility/ --authenticationDatabase admin
!

## 定时任务
:<<!
## 每天凌晨12点备份
0 0 * * * backup-mongodb-to-local.sh

## 每周六凌晨12点备份
0 0 * * 6 backup-mongodb-to-local.sh
